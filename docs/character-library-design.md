...
- 日期：2026-09-03
- 状态：Task 2 至 Task 9 已封板，第一阶段本地实现与验收完成，Task 10 远端交付等待授权
- 功能入口：`工作台 > 书籍 > 角色库`
- Stitch 项目：`小说分析台 · 角色库`
...
角色库构建以持久化 build ID 同时作为当前进程的 task ID，首次执行与跨进程恢复都以同一 ID 重建任务通道，避免公开 API 在 build 与 task 两套随机标识之间转换

构建状态、进度、错误和控制意图以数据库记录为事实源；SSE 重连后发送当前 snapshot 和后续事件，不承诺回放进程重启前的历史事件
...
- 隔离临时数据库建立两章，仅第 2 章具有匹配当前签名的新鲜 L1/L2 角色事实
- 两阶段 Task 4 使用 mock Dify 完成，构建结果为 1 个角色、1 个阶段和部分覆盖
- 独立随机端口 API 子进程从同一临时 `DATA_DIR` 读回当前 build，列表与详情的角色 ID、阶段 ID 和 build ID 一致
- 详情读回证明事实来自第 2 章，证据可读，稳定外形、稳定气质、原文五官、设计五官和设计依据分层保存
- 聚焦端到端测试、`test/service.test.js`、全仓测试、lint、Vite build 和 `git diff --check` 均通过，最终数量以 Task 9 控制卡封板记录为准
- 本地 API 已读回三本样板书当前 build、角色数、阶段数、事实链接、覆盖和质量摘要，证据分别写入三本书的 `runs/2026-09-02-character-library-r01/manifest.json`
- 《哥，别舔女主了！妹宝被你死对头亲晕了》覆盖 190/190 章，当前投影为 76 个角色、76 个阶段和 1297 条事实链接，6 个空 stages 候选保持失败
- 《凰宫梦》覆盖 2242/2243 章，当前投影为 790 个角色、801 个阶段和 21454 条事实链接，18 个空 stages 候选保持失败
- 《凰宫梦》结果显著高于 179 人正式基线，本次只确认链路、部分失败和多阶段存储可运行，不宣称角色召回与别名归并质量达标
- 《离婚后她惊艳了世界》覆盖 1155/3869 章，当前投影为 590 个角色、591 个阶段和 13777 条事实链接，27 个空 stages 候选保持失败；此前失败 build 未激活，当前可用投影得到保留
- B 方案已在 `1440x900` 和 `390x844` 验证表格、搜索筛选、状态提示与详情抽屉，浏览器控制台无错误或警告，截图保存在 `.ui-review/character-library/`
- 真实数据暴露非角色主体、可疑别名归并和性别值未归一化，统一记为后续数据质量治理，不在 Task 9 扩写 Task 2 或 Task 4 规则
